...
def generar_proximos():
    ...
    # TOP 1 Optimal Config Parameters (V6.1)
    pool_size = 14
    penalizacion_compartido = 0.1
    w_ind, w_cooc, w_esp, w_gap = (0.6, 0.18, 0.12, 0.1)
    
    top_k = sorted([x[0] for x in ranking[:pool_size]])
    print(f"Top {pool_size} Numeros: {top_k}")
    ...
    for boleto in boletos_reducidos:
        ...
        score_total = (score_ind * w_ind) + (score_cooc * w_cooc) + (score_espejo * w_esp) + (score_gap * w_gap)
        boletos_puntuados.append((boleto, score_total))
        
    boletos_puntuados.sort(key=lambda x: x[1], reverse=True)
    
    # Seleccionar los 3 boletos optimos
    tickets_optimos = seleccionar_mmr(boletos_puntuados, penalizacion_compartido)
    
    print("\n=== MODELO V6 OPTIMO: 3 TICKETS (MMR p=0.10) ===")
    ...
    nueva_prediccion = {
        ...
        "juego": "Quini 6 (Modelo V6.1 Optimo - 3 Tickets p=0.10)",
        "tickets": [
            {
                "nombre": f"SÚPER TICKET V6.1 OPTIMO {chr(65+idx)}",
                "numeros": sorted(b)
            } for idx, b in enumerate(tickets_optimos)
        ]
    }
